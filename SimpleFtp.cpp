...
//파일을 업로드한다
BOOL CSimpleFtp::UploadFile(CString strLocalFilePath, CString strRemoteFileDir, BOOL bResume)
{
	if (IsConnected() == FALSE)
	{
		if (Login() == FALSE)
	...
	if (strRemoteFileDir != _T("/"))
	{
		strRemoteFileDir.Replace(_T("\\"), _T("/"));
		strRemoteFileDir.Replace(_T("//"), _T("/"));

		if (CreateRemoteDir(strRemoteFileDir) <= 0)
		{
	...
	strRemoteFilePath = strRemoteFileDir + _T("/") + PathFindFileName(strLocalFilePath);
...
BOOL CSimpleFtp::UploadFile(CString strLocalFilePath, CString strRemoteFileDir, CString strFileName, BOOL bResume /*= FALSE*/)
{
	if (IsConnected() == FALSE)
	{
		if (Login() == FALSE)
	...
	CString strRemoteFilePath;
	strRemoteFilePath = strRemoteFileDir + _T("/") + strFileName;
	if (_T("/") != strRemoteFilePath.Left(1))
...
int CSimpleFtp::CreateRemoteDir(CString strRemoteDir)
{
	...
	if (strRemoteDir.GetLength() > 1 && strRemoteDir.Left(1) != _T('/'))
	...
	iCopy_End = strRemoteDir.Find(("/"), iCopy_Start);
	while (1)
	{
		strFolderName = strRemoteDir.Left(iCopy_End);
		iFolderCheck = FindRemoteDir(strFolderName);

		if (1 == iFolderCheck)
		{
			if (strRemoteDir.GetLength() == iCopy_End)
				break;

			iCopy_Start = iCopy_End + 1;
			iCopy_End = strRemoteDir.Find(_T("/"), iCopy_Start);
		}
		else if (0 == iFolderCheck)
		{
			if (FtpCreateDirectoryA(hFtpConn, GetSendString(strFolderName)) == FALSE)//디렉토리 생성을 시도
			{
				//디렉토리 생성 실패
				ShowLastErrorMsg();
				return 0;
		...
		if (-1 == iCopy_End)
			iCopy_End = strRemoteDir.GetLength();
	}
